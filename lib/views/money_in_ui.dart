// lib/screens/money_income_screen.dart
...
class _MoneyIncomeScreenState extends State<MoneyIncomeScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final txs = await _service.fetchIncome();
      setState(() {
        _transactions = txs;
        _isLoading = false;
      });
      ...
      builder: (context, child) {
        return Theme(
          data: Theme.of(context).copyWith(
            colorScheme: const ColorScheme.light(primary: Color(0xFF2DB89D)),
          ),
          child: child!,
        );
      },
    ...
    if (_transactions.isEmpty) {
      return const Center(
        child: Padding(
          ...
          child: Text('ยังไม่มีรายรับ', style: TextStyle(color: Colors.grey, fontSize: 16)),
        ),
      );
    }
    return Column(
